...
class SchedulerMobile extends Component {
    ...
    getTheSchedule() {
      ...
      fetch(`${Config.apiUrl}/wp-json/postlight/v1/products/daily_single_schedule?id=${this.props.productId}&date=${this.props.date}`, {
        method: 'GET',
      }).then(response => {
    ...
    render() {
      ...
      let menuItems = (this.state.responseLoaded) ?
          ...
          <div className="schedule-DayColumn" style={{display: 'flex', flex: '1', flexWrap: 'wrap', flexDirection: 'column', background: '#fff', border: '1px solid #ccc'}}>
            {this.state.schedule.slots.map((slot, index) => {
              if (slot.workers[0] !== '0') {
                return (
                  <div is
      ...
      let days = (this.state.responseLoaded) ? <div className="schedule-day" style={{flex: '1', border: '1px solid #000', fontFamily: 'Oswald, sans-serif', textAlign: 'center', color: '#fff', background: '#111'}}>{this.state.schedule.day + ', ' + this.state.schedule.date}</div> : '';

      let hours = (this.state.responseLoaded) ?
      this.state.schedule.slots.map((hour, index) => {
        return(
      ...
      : '';
        ...
        return (
          <div style={{maxWidth:'75vw', margin: '3em auto 1em', position: 'relative'}}>
            <div style={{height: '80vh', overflow: 'scroll'}}>
              ...
              <div style={{display: 'flex', fontFamily: 'Roboto, sans-serif', marginTop: '25px'}}>
                ...
                <div className="schedule-DayColumn" style={{display: 'flex', flex: ' 0 0 20%', flexWrap: 'wrap', flexDirection: 'column', background: '#fff', border: '1px solid #000'}}>
                  {hours}
                </div>
                {menuItems}
              </div>
